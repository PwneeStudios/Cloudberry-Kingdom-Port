...
namespace CloudberryKingdom
{
	...
	void StyleData::Randomize()
	{
		...
		Tools::Write( Format( _T( "Pre-style: %d" ), TestNumber ) );
		...
		TestNumber = Rnd->RndInt( 0, 1000 );
		Tools::Write( Format( _T( "Post-style: %d" ), TestNumber ) );
	}
	...
	void StyleData::InitializeInstanceFields()
	{
		...
		const float tempVector4[] = { 1, 1.5f, 2, 4 };
		...
		const float tempVector5[] = { .5f,.35f,.1f,.05f };
		...
		const float tempVector6[] = { .35f,.5f,.1f,.05f };
		...
		const float tempVector7[] = { .7f,.25f,.05f };
		...
		const float tempVector8[] = { .7f,.25f,.05f };
