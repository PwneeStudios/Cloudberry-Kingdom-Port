...
namespace CloudberryKingdom
{
	...
	void SpikeyGuy_Parameters::SetParameters( const std::shared_ptr<PieceSeedData> &PieceSeed, const std::shared_ptr<Level> &level )
	{
		...
		if ( PieceSeed->MyUpgrades1[ Upgrade_PINKY ] > 0 || PieceSeed->MyUpgrades2[ Upgrade_PINKY ] > 0 )
			Make = true;
		else
			Make = false;

		// General difficulty
		float FloaterLevel = PieceSeed->MyUpgrades1[ Upgrade_PINKY ];
		if ( FloaterLevel > 6 )
			NumOffsets = 8;
		else
			NumOffsets = 4;
...
const std::shared_ptr<SpikeyGuy_AutoGen> SpikeyGuy_AutoGen::instance = std::make_shared<SpikeyGuy_AutoGen>();
	...
	const std::shared_ptr<SpikeyGuy_AutoGen> &SpikeyGuy_AutoGen::getInstance() const
	{
		return instance;
	}
	...
	void SpikeyGuy_AutoGen::Cleanup_2( const std::shared_ptr<Level> &level, Vector2 BL, Vector2 TR )
	{
		...
		std::shared_ptr<SpikeyGuy_Parameters> Params = static_cast<SpikeyGuy_Parameters*>( level->Style_FIND_PARAMS( SpikeyGuy_AutoGen::getInstance() ) );
