#include "CloudberryKingdom.Obstacles.Fireball.h"
...
std::shared_ptr<ParticleEmitter> Fireball_Emitter, Flame_Emitter, Fireball::Emitter_Emitter = 0;
		...
		void Fireball::TexturePhsx()
		{
			...
			for ( int k = 0; k < 1; k++ )
			{
				std::shared_ptr<CloudberryKingdom::Particles::Particle> p = Flame_Emitter->GetNewParticle( EmitterTemplate );
		...
		void Fireball::Explosion( Vector2 pos, const std::shared_ptr<Level> &level, Vector2 vel, float Scale, float ScaleQuad )
		{
			...
			for ( int k = 0; k < 20; k++ )
			{
				std::shared_ptr<CloudberryKingdom::Particles::Particle> p = level->MainEmitter->GetNewParticle( ExplodeTemplate );
		...
		void Fireball::MakeNew()
		{
			...
			DeathType = Bobs::Bob::BobDeathType_FIREBALL;
