#include "CloudberryKingdom.Levels.Level.h"
...
namespace CloudberryKingdom
{
	namespace Levels
	{
		...
		void Level::MakeData::Init( const std::shared_ptr<PieceSeedData> &data )
		{
			int N = data->Paths;
		...
		std::vector<Bob*> Level::MakeData::MakeBobs( const std::shared_ptr<Level> &level )
		{
			/*
		...
		std::shared_ptr<LevelPiece> Level::MakeData::MakeLevelPiece( const std::shared_ptr<Level> &level, std::vector<Bob*> bobs, int Length, int StartPhsxStep )
		{
		...
		void Level::MakeData::InitializeInstanceFields()
		{
		...
		bool Level::MakeVertical( int Length, float Height, int StartPhsxStep, int ReturnEarly, const std::shared_ptr<MakeData> &makeData )
		{
			...
			CurMakeData->PieceSeed->Style_CALC_GEN_PARAMS( CurMakeData->PieceSeed, this );
			...
			if ( CurMakeData->FinalPlats )
			{
				if ( VStyle->MyFinalPlatsType == StyleData::FinalPlatsType_DOOR )
					MakeFinalPlat = std::make_shared<MakeFinalDoorVertical>( this );
				if ( VStyle->MyFinalPlatsType == StyleData::FinalPlatsType_DARK_BOTTOM )
					MakeFinalPlat = std::make_shared<MakeDarkBottom>( this );
			}
			...
			// Stage 1 fill
			std::shared_ptr<NormalBlock_Parameters> BlockParams = static_cast<NormalBlock_Parameters*>( VStyle->FindParams( NormalBlock_AutoGen::getInstance() ) );
			...
			if ( Geometry == LevelGeometry_UP )
			{
				std::shared_ptr<CloudberryKingdom::Blocks::NormalBlock> back = MakePillarBack( getFinalDoor()->getPos() + Vector2(0, -400), getFinalDoor()->getPos() + Vector2(0, 2000) );
				back->getBlockCore()->CeilingDraw = true;
				MakePillarBack( getStartDoor()->getPos() + Vector2(0, 400), getStartDoor()->getPos() - Vector2(0, 2000) );
			}
			else
			{
				std::shared_ptr<CloudberryKingdom::Blocks::NormalBlock> back = MakePillarBack( getStartDoor()->getPos() + Vector2(0, -400), getStartDoor()->getPos() + Vector2(0, 2000) );
				back->getBlockCore()->CeilingDraw = true;
			...
			switch ( Ladder )
			{
				case LadderType_FINAL_BOUNCY:
					...
					while ( y < LeftCenter.Y + getMainCamera()->GetHeight() / 2 - 400 )
					{
						...
						bouncy->Init( Vector2( LeftCenter.X, y ), Vector2( 220, 220 ), 70, this );
...
const float Level::SafetyNetHeight = 124;
		...
		std::shared_ptr<BlockBase> Level::Stage1SafetyNet( Vector2 BL, Vector2 TR, Vector2 size, float xstep, StyleData::GroundType Type )
		{
			...
			// Safety net
			__LastBlock.reset();
			Fill( BL + Vector2( 0, SafetyNetHeight ), Vector2( TR.X, BL.Y + SafetyNetHeight + 1 ), xstep, 50, std::make_shared<SafetyNetLambda>( this, BL, TR, size, xstep, Type, Virgin, Used, BoxesOnly, InvertDraw, Invert ) );
		...
		std::shared_ptr<Door> Level::PlaceDoorOnBlock( Vector2 pos, const std::shared_ptr<BlockBase> &block, bool AddBackdrop, const std::shared_ptr<TileSet> &BackdropTileset, bool LayeredDoor )
		{
			...
			// Add door
			std::shared_ptr<Door> door = static_cast<Door*>( getRecycle()->GetObject(ObjectType_DOOR, false) );
		...
		float Level::MakeInitial_Normal( Vector2 BL, Vector2 TR, Vector2 size )
		{
			...
			Fill( BL, Vector2( BL.X + 10, TR.Y ), 200, 250, std::make_shared<MakeInitialLambda>( this, size ) );
...
int Level::CountToSleep = 0;
		...
		void Level::Stage1RndFill( Vector2 BL, Vector2 TR, Vector2 BL_Cutoff, float Sparsity )
		{
			...
			xstep.Y = xstep.X;
			Fill( BL, TR, xstep, CurMakeData->PieceSeed->Style_FILLY_STEP, std::make_shared<Stage1RndFillLambda>( this, BL, TR, BL_Cutoff ) );
		}
...
int Step1, Level::Step2 = 0;
		...
		bool Level::MakeSingle( int Length, float MaxRight, float MaxLeft, int StartPhsxStep, int ReturnEarly, const std::shared_ptr<MakeData> &makeData )
		{
			...
			// Calculate the style parameters
			Style_CALC_GEN_PARAMS( CurMakeData->PieceSeed, this );
			...
			if ( CurMakeData->FinalPlats )
			{
				if ( Style_MY_FINAL_PLATS_TYPE == StyleData::FinalPlatsType_DOOR )
				{
					if ( DefaultHeroType == BobPhsxRocketbox::getInstance() || dynamic_cast<BobPhsxSpaceship*>(DefaultHeroType) != 0 )
						MakeFinalPlat = std::make_shared<MakeFinalDoor_Float>( this );
					else
						MakeFinalPlat = std::make_shared<MakeFinalDoor>( this );
				}
			}
			...
			// Door properties
			std::shared_ptr<Door> door = dynamic_cast<Door*>( FindIObject( LevelConnector::EndOfLevelCode ) );
			if ( 0 != door )
...
//C# TO C++ CONVERTER TODO TASK: There is no equivalent to implicit typing in C++ unless the C++11 inferred typing option is selected:
		...
		void Level::InitMakeData( const std::shared_ptr<MakeData> &makeData )
		{
			...
			{
				case 1:
					makeData->PieceSeed->Style_SET_SINGLE_PATH_TYPE( makeData, this, Piece );

					break;

				case 2:
					makeData->PieceSeed->Style_SET_DOUBE_PATH_TYPE( makeData, this, Piece );

					break;

				case 3:
					makeData->PieceSeed->Style_SET_TRIPLE_PATH_TYPE( makeData, this, Piece );
...
int Level::AfterPostDrawLayer = 12;
		...
		void Level::Release()
		{
			...
			if ( ParticleEmitters.size() > 0 )
				for ( std::vector<CloudberryKingdom::Particles::ParticleEmitter*>::const_iterator emitter = ParticleEmitters.begin(); emitter != ParticleEmitters.end(); ++emitter )
					if ( *emitter != 0 )
						( *emitter )->Release();
		...
		{
			return dynamic_cast<Door*>( FindIObject( LevelConnector::EndOfLevelCode ) );
		}

		const std::shared_ptr<Door> &Level::getStartDoor() const
		{
			return dynamic_cast<Door*>( FindIObject( LevelConnector::StartOfLevelCode ) );
		}
...
//C# TO C++ CONVERTER TODO TASK: There is no equivalent to implicit typing in C++ unless the C++11 inferred typing option is selected:
		...
		void Level::AddObject( const std::shared_ptr<ObjectBase> &NewObject, bool AddTimeStamp )
		{
			...
			NewObject->getCore()->MyLevel = this;
		...
		void Level::AddBlock( const std::shared_ptr<BlockBase> &block, bool AddTimeStamp )
		{
			...
			block->getBlockCore()->MyLevel = this;
...
std::vector<float> Level::BobLightRadiusByDifficulty = std::vector<float>( tempVector, tempVector + sizeof( tempVector ) / sizeof( tempVector[ 0 ] ) );
		...
		{
			Tools::RemoveAll( Objects, std::make_shared<RemoveForeignLambda>( this ) );
			for ( int i = 0; i < NumDrawLayers; i++ )
				Tools::RemoveAll( DrawLayer[ i ], std::make_shared<RemoveForeignLambda>( this ) );
...
//C# TO C++ CONVERTER TODO TASK: There is no equivalent to implicit typing in C++ unless the C++11 inferred typing option is selected:
		...
		void Level::SetBack( int Steps )
		{
			...
			MyGame->WaitThenDo( 2, std::make_shared<SetBackLambda>( this, Steps ) );
		}
